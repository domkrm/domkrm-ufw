# Creates a new reject rule in UFW
define ufw::reject (
  ...
  $_rule   = "reject in${_interface} proto ${proto}${_from} to any port ${port}"
  $_exists = "ufw status | grep -qE '^${_port_match}\\/${proto}${_interface_match} +REJECT +${_from_match}( +.*)?$'"

  if $ensure == 'present' {

    exec { "ufw-reject-${name}":
      command => "ufw ${_rule}",
      unless  => $_exists,
